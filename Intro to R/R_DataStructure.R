...
m1[,3]  # Show all elements in the 3rd column
...
a1*i1  # Element-wise multiplication
a1%*%i1  # Matrix multiplication
inv.a1 <- solve(a1)	# Inverse of a matrix
?solve
inv.a1
a2 <- chol(a1)  # Cholesky decomposition of a matrix
a2
t(a2)  # Transpose a matrix
t(a2)%*%a2
...
# []v.s [[]]: the class of the objects they return when used for extraction 
id <- c(11,12,13)
...
mtcars  # Show the Motor Trend data built in R
str(mtcars)  # Show the data strucuture of mtcars
attributes(mtcars)
head(mtcars)
head(mtcars,n=3)
tail(mtcars)
...
# Difference between two index forms: [[ vs. [
